"""OpenTelemetry metrics for tracking function usage in HoopGT.
...
Usage:
    ...
    >>> from hoopgt.telemetry.metrics import increment_counter
...
Configuration:
    ...
    >>> from hoopgt.telemetry.metrics import set_telemetry_metrics
...
function_counter = meter.create_counter(
    name="hoopgt.function.calls",
    description="Number of function calls",
    unit="1",
)

# Set initial metrics enabled state in env var if not already set
if "HOOPGT_METRICS_ENABLED" not in os.environ:
    os.environ["HOOPGT_METRICS_ENABLED"] = str(CONFIG["metrics_enabled"]).lower()
...
def is_metrics_enabled() -> bool:
    ...
    """
    return os.environ.get("HOOPGT_METRICS_ENABLED", "false").lower() == "true"
...
def set_telemetry_metrics(enabled: bool, set_as_default: bool = False) -> None:
    ...
    os.environ["HOOPGT_METRICS_ENABLED"] = str(enabled).lower()
    if set_as_default:
        _save_metrics_config(enabled)


def increment_counter(function_name: str, success: bool = True, hoopgt_config: Optional[str] = "") -> None:
    """
    ...
    hoopgt_config : str, optional
        A string representation of the optimization config used for the operation (default is an empty string).
    ...
    if is_metrics_enabled():
        function_counter.add(
            1,
            {
                ...
                "hoopgt_config": hoopgt_config,  # type: ignore
...
def track_usage(name_or_func: Optional[str | Callable] = None) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            hoopgt_config = kwargs.get("hoopgt_config")
            hoopgt_config = repr(hoopgt_config) if hoopgt_config is not None else ""
            try:
                result = func(*args, **kwargs)
                increment_counter(function_name, success=True, hoopgt_config=hoopgt_config)
                return result
            except Exception:
                increment_counter(function_name, success=False, hoopgt_config=hoopgt_config)
                raise
