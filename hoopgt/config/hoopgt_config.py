...
from hoopgt.config.optimize_space import ALGORITHM_GROUPS, OPTIMIZE_SPACE
from hoopgt.data.hoopgt_datamodule import HoopGTDataModule, TokenizerMissingError
from hoopgt.engine.utils import set_to_best_available_device
...
HOOPGT_CONFIG_FILE_NAME = "hoopgt_config.json"
SUPPORTED_DEVICES = ["cpu", "cuda", "mps", "accelerate"]


class HoopGTConfig:
    """
    Wrapper class to hold a ConfigSpace Configuration object as a HoopGT configuration.
    ...
    device : str | torch.device | None, optional
        The device to be used for optimization, options are "cpu", "cuda", "mps", "accelerate". Default is None.
    ...
    configuration : Configuration, optional
        The configuration to be used for optimization. If None, a default configuration will be created.
    """
    ...
    def __init__(
        ...
        cache_dir_prefix: str = os.path.join(os.path.expanduser("~"), ".cache", "hoopgt"),
        configuration: Configuration | None = None,
    ) -> None:
        OPTIMIZE_SPACE.gather_algorithm_buffer()
        self._configuration: Configuration = (
            OPTIMIZE_SPACE.get_default_configuration() if configuration is None else configuration
        )
        ...
        self.data: HoopGTDataModule | None = None

        # internal variable *to save time* by avoiding compilers saving models for inference-only smashing
        self._prepare_saving = True

        # internal variable to indicated that a model has been optimized for a specific batch size
    ...
    def __del__(self) -> None:
        """Delete the HoopGTConfig object."""
        self.cleanup_cache_dir()

    def __eq__(self, other: Any) -> bool:
        """Check if two HoopGTConfigs are equal."""
    ...
    def load_from_json(self, path: str | Path) -> None:
        """
        Load a HoopGTConfig from a JSON file.
        ...
        """
        with open(os.path.join(path, HOOPGT_CONFIG_FILE_NAME), "r") as f:
            json_string = f.read()
        ...
        for name in ADDITIONAL_ARGS:
            if name not in config_dict:
                hoopgt_logger.warning(f"Argument {name} not found in config file. Skipping...")
                continue
        ...
        for group in current_groups - expected_groups - applied_algorithm_args:
            if config_dict[group] is not None:
                hoopgt_logger.warning(
                    ...
                    "You can use an older version of HoopGT to load the model or reconfigure the model."
                )
        ...
        self._configuration = Configuration(OPTIMIZE_SPACE, values=config_dict)
    ...
    def save_to_json(self, path: str | Path) -> None:
        """
        Save the HoopGTConfig to a JSON file, including additional keys.
        ...
        with open(os.path.join(path, HOOPGT_CONFIG_FILE_NAME), "w") as f:
        ...
        if self.data is not None:
            hoopgt_logger.info("Data detected in hoopgt config, this will be detached and not reloaded...")

    def load_dict(self, config_dict: dict) -> None:
        """
        Load a dictionary of hyperparameters into the HoopGTConfig.

        Parameters
        ----------
        config_dict : dict
            The dictionary to load into the HoopGTConfig.
        ...
        if discarded_args:
            hoopgt_logger.info(f"Discarded arguments: {discarded_args}")
    ...
    def flush_configuration(self) -> None:
        """
        Remove all algorithm hyperparameters from the HoopGTConfig.
        ...
        HoopGTConfig()
        """
        self._configuration = OPTIMIZE_SPACE.get_default_configuration()
    ...
    def __get_dataloader(self, dataloader_name: str, **kwargs) -> torch.utils.data.DataLoader | None:
        ...
        if "batch_size" in kwargs and kwargs["batch_size"] != self.batch_size:
            hoopgt_logger.warning(
                ...
                f"set in the HoopGTConfig. Using the {self.batch_size}."
            )
    ...
    def train_dataloader(self, **kwargs) -> torch.utils.data.DataLoader | None:
        ...
        **kwargs : dict
        ...
        torch.utils.data.DataLoader | None
            The DataLoader instance associated with the HoopGTConfig.
        """
    ...
    def add_data(self, arg):
        ...
        """
        hoopgt_logger.error("Unsupported argument type for .add_data() HoopGTConfig function")
    ...
    def _(self, dataset_name: str, *args, **kwargs) -> None:
        try:
            ...
            self.data = HoopGTDataModule.from_string(dataset_name, *args, **kwargs)
        except TokenizerMissingError:
            raise ValueError(
                f"Tokenizer is required for {dataset_name} but not provided. "
                "Please provide a tokenizer with hoopgt_config.add_tokenizer()."
            ) from None
    ...
    def _(self, datasets: list, collate_fn: str, *args, **kwargs) -> None:
        try:
            kwargs["tokenizer"] = self.tokenizer
            self.data = HoopGTDataModule.from_datasets(datasets, collate_fn, *args, **kwargs)
        except TokenizerMissingError:
            raise ValueError(
    ...
    @add_data.register(HoopGTDataModule)
    def _(self, datamodule: HoopGTDataModule) -> None:
        self.data = datamodule

    def add_tokenizer(self, tokenizer: str | PreTrainedTokenizerBase) -> None:
        """
        Add a tokenizer to the HoopGTConfig.

        Parameters
        ----------
        tokenizer : str | transformers.AutoTokenizer
            The tokenizer to be added to the HoopGTConfig.
        """
    ...
    def add_processor(self, processor: str | ProcessorMixin) -> None:
        """
        Add a processor to the HoopGTConfig.

        Parameters
        ----------
        processor : str | transformers.AutoProcessor
            The processor to be added to the HoopGTConfig.
        """
    ...
    def lock_batch_size(self) -> None:
        """Lock the batch size in the HoopGTConfig."""
        self.__locked_batch_size = True

    def is_batch_size_locked(self) -> bool:
        """
        Check if the batch size is locked in the HoopGTConfig.
    ...
    def __getitem__(self, name: str) -> Any:
        ...
        if name in ADDITIONAL_ARGS:
            ...
            # we convert this to native python types for printing and handing arguments to optimize algorithms
    ...
    def __setitem__(self, name: str, value: Any) -> None:
        ...
        if name in deprecated_hyperparameters:
            warn(
                f"The {name} hyperparameter is deprecated. You can use HoopGTConfig(batch_size={value}) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
...
class HoopGTConfigPrefixWrapper:
    """
    Wrapper for HoopGTConfig to add a prefix to the config keys.

    Parameters
    ----------
    base_config : Union[HoopGTConfig, "HoopGTConfigPrefixWrapper"]
        The base HoopGTConfig or HoopGTConfigPrefixWrapper object.
    prefix : str
        The prefix to add to the config keys.
    """

    def __init__(self, base_config: Union[HoopGTConfig, "HoopGTConfigPrefixWrapper"], prefix: str) -> None:
